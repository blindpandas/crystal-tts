...
class WavePlayer:
    ...
    def _feedUnbuffered_handleErrors(self, data, onDone=None) -> bool:
        ...
        try:
            ...
            try:
                ...
                log.debugWarning(
                    "Unable to send data to audio device on second attempt.",
                    exc_info=True,
                )
                return False
        ...
        with self._lock:
            with self._waveout_lock:
                ...
                if self._prevOnDone is not self.STOPPING:
                    ...
                    with self._global_waveout_lock:
                        winmm.waveOutPrepareHeader(
                            self._waveout_arg, LPWAVEHDR(whdr), sizeof(WAVEHDR)
                        )
                        winmm.waveOutWrite(
                            self._waveout_arg, LPWAVEHDR(whdr), sizeof(WAVEHDR)
                        )
    ...
    def sync(self):
        ...
        with self._lock:
            ...
            while (
                ...
                winKernel.waitForSingleObject(
                    self._waveout_event, self._waveout_event_wait_ms
                )
            with self._waveout_lock:
        ...
        with self._lock:
            ...
            with self._waveout_lock:
                ...
                if self.closeWhenIdle:
                    ...
                    with self._global_waveout_lock:
                        if (
                            not self._isPreferredDeviceOpen()
                            and self._isPreferredDeviceAvailable()
                        ):
                            if _isDebugForNvWave():
    ...
    def _close(self):
        ...
        with self._global_waveout_lock:
            ...
            try:
                ...
                winmm.waveOutClose(self._waveout_arg)
            except WindowsError:
                log.debug(
                    "Error closing the device, it may have been removed.", exc_info=True
                )
        self._waveout = None
    ...
    def _safe_winmm_call(
        self, winmmCall: Callable[[Optional[int]], None], messageOnFailure: str
    ) -> bool:
        if not self._waveout:
            return False
        try:
            # ???
            winmmCall(self._waveout_arg)
            return True
        except WindowsError:
...
def playWaveFile(
    fileName: str, asynchronous: bool = True, isSpeechWaveFileCommand: bool = False
):
